...
class Promotions(Extension):
    ...
    @slash_command(name="promotions", description="Promotion management commands", sub_cmd_name="promote", sub_cmd_description="Promote a member")
    @slash_option(
        name="member",
    ...
    async def promotions(self, ctx, member: User, new_role: Role, reason: str = None):
    ...
    @promotions.subcommand(sub_cmd_name="view", sub_cmd_description="View promotion details by Promotion ID")
    @slash_option(
        name="promotion_id",
    ...
    async def view_promotion(self, ctx, promotion_id: str):
        await ctx.defer(ephemeral=True)
        ...
        if not new_role:
            try:
                ...
                new_role = None
        issued_by = ctx.guild.get_member(int(promotion_data["issued_by_id"]))
        if not issued_by:
            try:
                issued_by = await self.bot.fetch_user(int(promotion_data["issued_by_id"]))
            except Exception:
                issued_by = None
        ...
        await ctx.send(
            embed={
                ...
                "fields": [
                    {"name": "Member", "value": f"{member} ({member.id})" if member else f"Member not found ({promotion_data['member_id']})", "inline": True},
                    {"name": "New Role", "value": f"@{new_role.name} ({new_role.id})" if new_role else f"Role not found ({promotion_data['new_role_id']})", "inline": True},
